...
from (
	...
	from pdb_VT_ChronicPain..Com_MemberSummary_TTL_woUBH	m
...
--3,576,198
--3,574,468 - woUBH 10mins
create unique index uid_Indv on #SRT_Members_Matt (Indv_Sys_Id)
...
--Dynamic Pivot:
if OBJECT_ID ('pdb_VT_ChronicPain..Com_tmpPvtDiag_MattwoUBH') is not null begin drop table pdb_VT_ChronicPain..Com_tmpPvtDiag_MattwoUBH	End;
...
into pdb_VT_ChronicPain..Com_tmpPvtDiag_MattwoUBH
from
...
(select distinct Indv_Sys_Id
	...
	execute (@query1)
--293,936 1min
 create unique index uid_Indv on pdb_VT_ChronicPain..Com_tmpPvtDiag_MattwoUBH (Indv_Sys_Id)

--select count(distinct Indv_Sys_Id) from #SRT_Members_Matt
select top 1000 * from pdb_VT_ChronicPain..Med_tmpPvtDiag_Matt
--drop table pdb_VT_ChronicPain..Com_SRT_Memberspvt_Matt_woUBH
...
into pdb_VT_ChronicPain..Com_SRT_Memberspvt_Matt_woUBH
...
--293,941
create unique index uid_Indv on pdb_VT_ChronicPain..Com_SRT_Memberspvt_Matt_woUBH (Indv_Sys_Id)

-----------------------------------------------------------------------------------------------
select top 1000 * from pdb_VT_ChronicPain..Com_SRT_Memberspvt_Matt_woUBH
select DIAG_CD, rtrim(ltrim(DIAG_CD)) from #tmp_mbrs where DIAG_CD = 'Z9103'

select count(*) from pdb_VT_ChronicPain..Com_MemberSummary_TTL_woUBH where wChronicPain = 1
select top 1000 * from pdb_VT_ChronicPain..Com_MemberSummary_TTL_woUBH where wChronicPain = 1
...
--drop table pdb_VT_ChronicPain..Com_ChronicPain_type_app2_woUBH
select distinct DIAG_CD, DIAG_DESC, AHRQ_DIAG_DTL_CATGY_NM  into pdb_VT_ChronicPain..Com_ChronicPain_type_app2_woUBH from pdb_VT_ChronicPain..pain_types_v2 where DIAG_CD in (
...
--122
create unique index uid_dx on pdb_VT_ChronicPain..Com_ChronicPain_type_app2_woUBH (DIAG_CD)
...
--782,451
create unique index uid_Indv on #mmbrSRT (Indv_Sys_Id)

alter table pdb_VT_ChronicPain..Com_MemberSummary_TTL_woUBH
add DiagV2_SRTFlag_woUBH smallint

update pdb_VT_ChronicPain..Com_MemberSummary_TTL_woUBH set 
DiagV2_SRTFlag_woUBH = isnull(b.Diag_CPInt_SRTFlag,0)
from pdb_VT_ChronicPain..Com_MemberSummary_TTL_woUBH		a
